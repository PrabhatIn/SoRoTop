...
s1 = elNrs(1:nely/20,1:nelx/20);                                      % Solid element or element with rho =1
[NDS, NDV ] = deal( s1, [] );
...
PF([Tnode, Rnode]) = 0; PF(Lnode) = Pin; % applying pressure load
...
fixedUdofs = [2*Tnode(1:nely/20)-1  2*Tnode(1:nely/20)  2*Bnode]; %fixed displ.
...
[objb, objsensb,volb,volsensb, ~, ~,Ub,Fb,PFb] = ObjCnst_ObjCnst_Sens(xphysb,nel,E1,Emin,penal,Kv,Kvs,epsf,Ds,etaf,betaf,Udofs,freeUdofs,...
Pdofs,pfixeddofsv,fixedPdofs,freePdofs,iP,jP,iT,jT,iK,jK,Kp,KDp,Te,Ke,opdof,kss,loop,IFprj,dIFprj,L,U,lam1,lam2,mu1,lst,volfrac,sefrac);
%___PART 6.3___________Performing eroded design analysis using ObjObjSens function
[obje, objsense,~,~, SEe, SEsense,~,~,~] = ObjCnst_ObjCnst_Sens(xphyse,nel,E1, Emin,penal,Kv,Kvs,epsf,Ds,etaf,betaf,Udofs,freeUdofs, ...
Pdofs,pfixeddofsv,fixedPdofs,freePdofs,iP,jP,iT,jT,iK,jK,Kp,KDp,Te,Ke,opdof,kss,loop,IFprj,dIFprj,L,U,lam1,lam2,mu1,lst,volfrac,sefrac);
...
patch(X, Y1,1-xphysb,'EdgeColor','none');caxis([0 1]);
for i = 1:nel
...
patch(X(:,i), Y1(:,i),1-elemP(i),'FaceColor','w','EdgeColor','none')
end
end
%% ______PART 8____Plotting deformed profile____________________________
DFP = figure(3); set(DFP,'color','w'); axis equal off, hold on;colormap(gray);caxis([0 1]);
xn= node;                             % defomed nodal position
xn(:,2) = node(:,2) + 0.0025*Ub(1:2:end); xn(:,3) = node(:,3) + 0.0025*Ub(2:2:end);
Xn = reshape(xn(elem(:,2:5)',2),4,nel); Yn = reshape(xn(elem(:,2:5)',3),4,nel);
Yn1 = 2*min(node(:,3))-reshape(xn(elem(:,2:5)',3),4,nel); % for symmetry about x-axis
patch(Xn, Yn,  1-xphysb,'EdgeColor','none');
patch(Xn, Yn1, 1-xphysb,'EdgeColor','none');
for i = 1:nel
if (xphysb(i)<0.2 && elemP(i)>0.70)
...
patch(Xn(:,i), Yn1(:,i),1-elemP(i),'FaceColor','w','EdgeColor','none');
end
end
%% ___________PART 9_______ Analyses function
function[obj,objsens,vol,volsens, SEc, SEsens,U,F,PF] =ObjCnst_ObjCnst_Sens(xphys,nel,E1,Emin,penal,Kv,kvs,epsf,Ds,etaf,betaf,Udofs,...
freeUdofs,Pdofs,pfixeddofsv,fixedPdofs,freePdofs,iP,jP,iT,jT,iK,jK,Kp,KDp,Te,Ke,opdof,kss,loop,IFprj,dIFprj,L,U,lam1,lam2,mu1,lst,volfrac,sefrac)
% ___PATT 9.1_______SOLVING FLOW BALANCE EQUATION
...
%__PART 9.3______objective evaluation
obj = L'*U; % maximizing the output deformation
%__PART 9.4__________sensitivity analysis
