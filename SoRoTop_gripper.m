function SoRoTop_gripper(nelx,nely,volfrac,sefrac,penal,rmin,ks,etaf,betaf,lst,betamax,delrbst,maxit)
...
s2 = elNrs(3*nely/4:4*nely/5,4*nelx/5:nelx);
v1 = elNrs(4*nely/5:nely,4*nelx/5:nelx);
s = union (s1,s2);
[NDS, NDV ] = deal( s, v1 );
act = setdiff((1 : nel)', union( NDS, NDV ));
opelem = elNrs(3*nely/4,nelx); % finding the output element
opnode = Pdofs(opelem,2); % finding the output node
opdof = 2*opnode; % output degree of freedom
...
patch(Xn, Yn1, 1-xphysb,'EdgeColor','none');caxis([0 1]);
for i = 1:nel
     if (xphysb(i)<0.2 && elemP(i)>0.70)
